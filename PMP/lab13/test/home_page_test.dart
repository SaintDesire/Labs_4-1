...
void main() {
  ...
  testWidgets('HomePage scrollable horizontal drag test', (WidgetTester tester) async {
    await tester.pumpWidget(const MaterialApp(home: HomePage()));

    expect(find.byType(SingleChildScrollView), findsWidgets);

    await tester.drag(find.byType(SingleChildScrollView).last, Offset(-200, 0));

    await tester.pump();

    expect(find.text('Nike GTX'), findsWidgets);
  });
